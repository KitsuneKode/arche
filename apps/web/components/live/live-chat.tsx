...
export function LiveChat({ signedIn, userId }: { signedIn: boolean; userId?: string }) {
  ...
  const { pollingFallback } = useChatStream(invalidateChat, signedIn || true)

  const messagesQuery = useQuery({
    ...trpc.chat.list.queryOptions(),
    refetchInterval: () => {
      if (typeof document !== 'undefined' && document.hidden) return false
      // SSE invalidates on new messages; poll only when the stream is unavailable.
      return pollingFallback ? 3_000 : false
    },
  })
