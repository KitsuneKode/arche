...
- Architecture completion pass: shared `_web-core` template source with sync checks, generated web parity for Solana/fullstack scaffolds, runtime smoke verification for HTTP presets, TanStack Start and OpenTUI template families, atomic scaffold rollback, dry-run fidelity, truthful registry verification matrix, and CI combo-matrix plus pnpm next-app gates.
