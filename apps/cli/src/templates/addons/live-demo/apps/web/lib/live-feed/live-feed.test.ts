    ...
    handlers.get('chat:message')?.({
      data: JSON.stringify({
        message: {
          ...
          sender: { id: 'u1', name: 'Guest', image: null },
        },
      }),
    })
    handlers.get('game:leaderboard')?.({ data: JSON.stringify({ ok: true }) })
    feed.stop()

    expect(events).toEqual(['lattice:state', 'chat:message', 'game:leaderboard'])
  })
